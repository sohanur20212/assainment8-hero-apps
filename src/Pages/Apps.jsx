...
const Apps = () => {
  ...
  // Update searchApps whenever apps or search changes
  useEffect(() => {
    const term = search.trim().toLowerCase();
  ...
  const isNotFound = search && searchApps.length === 0;
    ...
    <div className='my-10'>
      ...
      {isNotFound ? (
        <div className='text-center mt-10 space-y-4'>
          ...
          <Link
            to='/'
            className='bg-gradient-to-r from-[#632EE3] to-[#9F62F2] px-4 py-1 rounded-md text-white'
          >
            Back to Home
          </Link>
        </div>
